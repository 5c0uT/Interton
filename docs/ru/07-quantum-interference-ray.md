...
- Тип -> `quant_i`, Что хранит -> квантовое состояние с целочисленной интерпретацией измерения, Пример значения ->  `H(|0>)`, then `measure(...) -> int`, Когда применять -> дискретные квантовые сценарии с выводом в `int`, Статус -> публичный
- Тип -> `quant_f`, Что хранит -> квантовое состояние с числовой (float) интерпретацией амплитуд/состояния, Пример значения ->  `|0>, |1>`, Когда применять -> смешанные расчеты, где нужен float-контур, Статус -> публичный
- Тип -> `qubit`, Что хранит -> одиночный квантовый контейнер состояния, Пример значения ->  `qubit q = |0>`, Когда применять -> моделирование отдельного кубита, Статус -> публичный
- Тип -> `gate`, Что хранит -> описание/маркировка квантовой операции, Пример значения -> `gate g = H`, Когда применять -> построение/описание квантовых шагов, Статус -> публичный (ограниченный runtime-смысл)
- Тип -> `quant_b`, Что хранит -> булев квантовый контейнер (internal-уровень), Пример значения ->  `-`, Когда применять -> прямой пользовательский синтаксис не использовать, Статус -> internal/reserved
